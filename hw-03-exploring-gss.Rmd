...
***The possible responses to the this question is yes, no and N/A***
...
***The percentage of respondents for whom this question is applicable is 42 percent***
```{r}
gss16 |>
  filter(!(harass5 == "Yes")) |>
  nrow()
...
4.  Visualize the distribution of this new variable.
    ...
    Why?
***The mean number of minutes spent on email per week is 417 mins. The median number of minutes spent on email per week is 120 mins. Mean is the better measure of the typical amount of time Americans spend on email weekly because the data is skewed***
```{r}
gss16 |>
  summarise(
    mean = mean(email, na.rm = emailmin),
    emailhr
  )
...
7.  What are the possible responses to the question *Last week were you working full time, part time, going to school, keeping house, or what?* and how many respondents chose each of these answers?
  ...
  ***The possible responses for this question are: Keeping house	284, Other	89, Retired	574, School	76, Temp not working	57, Unempl, laid off	118, Working fulltime	1321, Working parttime	345, and NA	3.***
```{r}
gss16 |>
  count(wrkstat)
```
